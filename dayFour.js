...
// let testArr = [
//     {
//         eyr: "2021", pid: "062406624", hcl: "#cff07d", byr: "1947", iyr: "2012", ecl: "hzl", hgt: "50cm",
//     },
//     {
//         iyr: "2013", byr: "1997", hgt: "182cm", hcl: "#ceb3a1",
//         eyr: "2027",
//         ecl: "oth", cid: "102", pid: "018128535",
//     },
//     {
//         iyr: "2013", byr: "1997", hgt: "182cm", hcl: "#cfb3a1",
//         eyr: "2027",
//         ecl: "blu", cid: "102", pid: "181285351",
//     }
// ]
...
for (let i = 0; i < newArr.length; i++) {
    if (newArr[i].hasOwnProperty("eyr") &&
        newArr[i].hasOwnProperty("pid") &&
        newArr[i].hasOwnProperty("hcl") &&
        newArr[i].hasOwnProperty("byr") &&
        newArr[i].hasOwnProperty("iyr") &&
        newArr[i].hasOwnProperty("ecl") &&
        newArr[i].hasOwnProperty("hgt")
    ) {
        validArr.push(newArr[i]);
    }
}
...
const testHgt = function (obj) {
    // If cm, the number must be at least 150 and at most 193.
    // If in, the number must be at least 59 and at most 76.
    let height = obj.hgt;
    let unit = height.slice(-1);
    let parNum = parseInt(height);

    if (unit === "m") {
        if (parNum >= 150 && parNum <= 193) {
            return true;
        }
        else {
            return false;
...
const testHcl = function (obj) {
    let hairColor = obj.hcl;
    let hairNum = hairColor.substr(1, 6);

    if (hairColor[0] === "#") {
        regexp = /^[0-9a-fA-F]+$/;

        if (regexp.test(hairNum)) {
            return true;
        }
        else {
            return false;
...
const testEcl = function (obj) {
    // ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
    if (obj.ecl === "amb" || obj.ecl === "blu" || obj.ecl === "brn" || obj.ecl === "gry" || obj.ecl === "grn" || obj.ecl === "hzl" || obj.ecl === "oth") {
        return true;
    }
    else {
        return false;
...
for (let i = 0; i < validArr.length; i++) {
    if (testByr(validArr[i])) {
        if (testIyr(validArr[i])) {
            if (testEyr(validArr[i])) {
                if (testHgt(validArr[i])) {
                    if (testHcl(validArr[i])) {
                        if (testEcl(validArr[i])) {
                            if (testPid(validArr[i])) {
                                testedArr.push(validArr[i]);
                            }
                        }
                    }
                }
